...
public class _4_과일가게_DB연결 {
	...
	public static void addFruit() {
		...
		try {
			...
			if (rs.next()) {
				...
				String insertSql = "INSERT INTO FRUIT VALUES (" + fruitName + ", " + price + ", " + cnt + ")";
	...
	public static void updateFruit() {
		...
		try {
			...
			if (rs.next()) {
				...
				String updateSql = "UPDATE FRUIT SET PRICE = " + newPrice + "WHERE FRUIT_NAME = " + fruitName;
	...
	public static void deleteFruit() {
		...
		try {
			...
			String sql1 = "SELECT * FROM FRUIT WHERE FRUIT_NAME = " + fruitName;
			...
			if (rs1.next()) {
				...
				if (result.equals("Y")) {
					...
					String sql2 = "DELETE FROM FRUIT WHERE FRUIT_NAME = " + fruitName;
